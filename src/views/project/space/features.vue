...
export default {
  ...
  methods: {
            init() {
                ...
                this.listData1.unshift(...this.listData)
            },
    //         getProject() {
    //             this.loading = true;
    ...
    filte() {
      ...
      if (this.showMe) {
        ...
        let res = this.listData.filter((val) => {
          return val.title == user;
        });
        this.listData1.splice(0)
        this.listData1.push(...res)
      }else{
        this.listData1.splice(0);
        this.listData1.push(...this.listData)
      }
    },
    exportExcel() {
      require.ensure([], () => {
    ...
    showModal1() {
     ...
     handleOk1() { 
       this.listData.unshift({
          ...
          time:'发表于 时间'+moment(new Date).format('YYYY-MM-DD HH:mm:ss'),
